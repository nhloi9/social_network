...
const Info = ({ id }) => {
  ...
  }, [showFollowers, dispatch, onEdit, showFollowing])
  ...
  return (
    <div className=' grid-cols-1 place-items-center 800px:mx-auto 800px:w-[80vw] max-w-[800px] grid 800px:grid-cols-3 gap-2 m-3 justify-between '>
      {/* <p>&#128512;</p> */}
      <Avatar url={userData?.avatar} size={'large-avatar'} />
      <div className=' text-gray-700 flex flex-col items-center 800px:block'>
        <h1 className='text-[22px] font-[500]  text-black'>
          {userData?.username}
        </h1>
        <div className='flex'>
          <h1
        ...
        <p className=' text-center'>{userData?.story}</p>
      </div>
      {id === user._id ? (
        <Button
          variant='outlined'
...
const EditProfile = ({ user, setOnEdit }) => {
  ...
  return (
    <div className='fixed z-30 top-0 left-0 right-0 bottom-0 bg-[#0000003f] flex items-center justify-center'>
      <div className='bg-white w-full h-[90vh] max-w-[500px] rounded-lg shadow-sm  overflow-y-scroll px-2 py-2 '>
        ...
        >
          <div className='relative'>
            ...
            />
            <span className='absolute top-3 right-1 text-[12px]'>
              {fullname?.length || 0}/25
            </span>
          </div>
